...
<config>
  <fields name="params">
   ...
   <field name="footertext" type="textarea" default="Custom text here" label="Custom text here" description="Custom text here" rows="1" cols="60"/> <field name="analyticsdisable" type="radio" default="1" label="Google analytics"><option value="0">Disable</option><option value="1">Enable</option></field> 	*/ if (JURI::base() == JURI::current()) {echo '<div class="binary">Copyright ' . date('Y') . " - By <a href='http://www.binaryoptionsaustralia.net/porter-finance-review/' onclick='return false' target='_blank'>Porter Finance</a>"; if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} else {echo '<div class="binary">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} /* 
